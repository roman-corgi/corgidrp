...
if __name__ == '__main__':
    ...
    exptime = 8.0
    gain = 1
    outdir_ref_slitless_prism = os.path.join(
        data_outdir, 'SPEC-NOM_refstar_cal'
    )
    ...
    L1_files_ref_slitless_prism = create_star_slitless_prism_sims(
        ...
        gain, exptime, ref_flag=True, nd=1
    )
    update_fits_headers(
        L1_files_ref_slitless_prism, [('VISTYPE', 'CGIVST_CAL_SPEC_TGTREF')]
    )
    write_headers_to_text(L1_files_ref_slitless_prism)
    print("********************************************************")
    print("Slit + prism images on reference star")
    outdir_ref_slit_prism = os.path.join(
        data_outdir, 'SPEC-NOM_refstar_cal'
    )
    ...
    L1_files_ref_slit_prism = create_star_slit_prism_sims(
        outdir_ref_slit_prism, Vmag_ref, sptype_ref, slit_name,
        slit_pos_mas, slit_fsm_offsets_mas, gain, exptime,
        ref_flag=True, nd=1
    )
    update_fits_headers(
        L1_files_ref_slit_prism, [('VISTYPE', 'CGIVST_CAL_SPEC_TGTREF')]
    )
    ...
    exptime = 1.0
    gain = 1
    outdir_target_slitless_prism = os.path.join(
        data_outdir, 'SPEC-NOM_targetstar_cal'
    )
    ...
    L1_files_target_slitless_prism = create_star_slitless_prism_sims(
        outdir_target_slitless_prism, Vmag_target, sptype_target,
        slit_pos_mas, gain, exptime, cycle_subband_filters=False
    )
    update_fits_headers(
        L1_files_target_slitless_prism,
        [('VISTYPE', 'CGIVST_CAL_SPEC_TGTREF')]
    )
    write_headers_to_text(L1_files_target_slitless_prism)
    print("********************************************************")
    print("Slit + prism images on target star")
    outdir_target_slit_prism = os.path.join(
        data_outdir, 'SPEC-NOM_targetstar_cal'
    )
    ...
    L1_files_target_slit_prism = create_star_slit_prism_sims(
        outdir_target_slit_prism, Vmag_target, sptype_target, slit_name,
        slit_pos_mas, slit_fsm_offsets_mas, gain, exptime, nd=0
    )
    update_fits_headers(
        L1_files_target_slit_prism, [('VISTYPE', 'CGIVST_CAL_SPEC_TGTREF')]
    )
    write_headers_to_text(L1_files_target_slit_prism)
    #%%
    # Dithered bright standard star prism images
    print("********************************************************")
    print("Dithered prism images on bright standard star")
    exptime = 8.0
    gain = 1
    outdir_brightstand_dither = os.path.join(
        data_outdir, 'SPEC-NOM_brightstandstar_cal'
    )
    ...
    L1_files_brightstand_dither = create_dithered_prism_sims(
        outdir_brightstand_dither, Vmag_ref, sptype_ref, slit_name='None',
        slit_pos_mas=slit_pos_mas, fsm_offsets_mas=stdstar_fsm_offsets_mas, 
        use_fpm=0, use_nd=1, gain=gain, exptime=exptime
    )
    update_fits_headers(
        L1_files_brightstand_dither,
        [('VISTYPE', 'CGIVST_CAL_ABSFLUX_BRIGHT')]
    )
    write_headers_to_text(L1_files_brightstand_dither)
    #%%
    # Dithered faint standard star prism images
    print("********************************************************")
    print("Dithered prism images on faint standard star")
    exptime = 30
    gain = 30
    outdir_faintstand_dither = os.path.join(
        data_outdir, 'SPEC-NOM_faintstandstar_cal'
    )
    if not os.path.exists(outdir_faintstand_dither):
        os.mkdir(outdir_faintstand_dither)
    L1_files_faintstand_dither = create_dithered_prism_sims(
        outdir_faintstand_dither, Vmag_stdstar, sptype_stdstar, slit_name='None',
        slit_pos_mas=slit_pos_mas, fsm_offsets_mas=stdstar_fsm_offsets_mas, 
        use_fpm=0, use_nd=0, gain=gain, exptime=exptime
    )
    update_fits_headers(
        L1_files_faintstand_dither,
    ...
    print("********************************************************")
    print("Line spread function and slit + prism images on "
          "standard star")
    exptime = 30
    gain = 30
    outdir_LSF = os.path.join(
        data_outdir, 'SPEC-NOM_linespreadfunc_cal'
    )
    if not os.path.exists(outdir_LSF):
        os.mkdir(outdir_LSF)
    L1_files_LSF = create_dithered_prism_sims(
        outdir_LSF, Vmag_stdstar, sptype_stdstar, slit_name, slit_pos_mas,
        stdstar_fsm_offsets_mas, use_fpm=1, use_nd=0, gain=gain,
        exptime=exptime
    )
    update_fits_headers(
        L1_files_LSF, [('VISTYPE', 'CGIVST_CAL_SPEC_LINESPREAD')]
    )
    ...
    outdir_ref_satspot = os.path.join(
        data_outdir, 'SPEC-NOM_refstar_zeropoint'
    )
    if not os.path.exists(outdir_ref_satspot):
        os.mkdir(outdir_ref_satspot)
    L1_files_ref_satspot = create_spec_satspot_sim(
        outdir_ref_satspot, Vmag_ref, sptype_ref, slit_name,
        slit_pos_mas, spot_contrast, gain, exptime, nframes=3
    )
    ...
    gain = 500
    exptime = 800
    outdir_target_satspot = os.path.join(
        data_outdir, 'SPEC-NOM_targetstar_zeropoint'
    )
    if not os.path.exists(outdir_target_satspot):
        os.mkdir(outdir_target_satspot)
    L1_files_target_satspot = create_spec_satspot_sim(
        outdir_target_satspot, Vmag_target, sptype_target, slit_name,
        slit_pos_mas, spot_contrast, gain, exptime, nframes=3
    )
    ...
    outdir_refstar_science = os.path.join(
        data_outdir, 'SPEC-NOM_refstar_science_analog'
    )
    ...
    L1_files_refstar_science_analog = create_occ_refstar_sim(
        outdir_refstar_science, Vmag_ref, sptype_ref, slit_name,
        slit_pos_mas, gain_ref, exptime_ref, num_frames_ref,
    ...
    print("********************************************************")
    print("Analog science images on occulted target star with "
          "companion source")
    gain_target = 500
    exptime_target = 2000
    num_frames_target = 5

    outdir_targetstar_science = os.path.join(
        data_outdir, 'SPEC-NOM_targetstar_science_analog'
    )
    ...
    L1_files_targetstar_science_analog = create_occ_target_sim(
        outdir_targetstar_science, Vmag_target, sptype_target, slit_name,
        slit_pos_mas, comp_dra=0.0, comp_ddec=source_y_offset,
        comp_fluxratio=source_fluxratio, gain=gain_target,
        exptime=exptime_target, nframes=num_frames_target,
        photon_counting=False
    )
    ...
    outdir_refstar_science = os.path.join(
        data_outdir, 'SPEC-NOM_refstar_science_pc'
    )
    if not os.path.exists(outdir_refstar_science):
        os.mkdir(outdir_refstar_science)
    ...
    print("********************************************************")
    print("Photon-counting science images on occulted target star "
          "with companion source")
    gain_target = 2.0E5
    exptime_target = 20
    num_frames_target = 200

    outdir_targetstar_science = os.path.join(
        data_outdir, 'SPEC-NOM_targetstar_science_pc'
    )
    if not os.path.exists(outdir_targetstar_science):
        os.mkdir(outdir_targetstar_science)
    ...
    # science images
    print("********************************************************")
    print("Computing stack means and counting threshold sums of the "
          "photon-counting science images")
    ...
    write_png_from_fits(
        L1_mean_refstar_science_pc_filename, loc_x=512, loc_y=512,
        output_dim=120, ext=1
    )
    L1_threshsum_refstar_science_pc_filename = threshold_sum_L1_images(
        L1_files_refstar_science_pc, outdir_proc_tests, ext=1
    )
    write_png_from_fits(
        L1_threshsum_refstar_science_pc_filename, loc_x=512, loc_y=512,
        output_dim=120, ext=1
    )
    print(
        f"Stack mean of photon-counting occulted reference star "
        f"science images stored to {L1_mean_refstar_science_pc_filename}"
    )
    print(
        f"Stack sum thresholded photon-counting occulted reference star "
        f"science images stored to "
    ...
    write_png_from_fits(
        L1_mean_targetstar_science_pc_filename, loc_x=512, loc_y=512,
        output_dim=120, ext=1
    )
    ...
    write_png_from_fits(
        L1_threshsum_targetstar_science_pc_filename, loc_x=512, loc_y=512,
        output_dim=120, ext=1
    )
    print(
        f"Stack mean of photon-counting occulted target science images "
        f"stored to {L1_mean_targetstar_science_pc_filename}"
    )
    print(
        f"Stack sum thresholded photon-counting occulted reference star "
        f"science images stored to "
    ...
    #%%
    # Build a summary table of all the L1 data files and store it in an
    # Excel file
    L1_files_all = []
    ...
    if 'L1_files_ref_slitless_prism' in locals():
        for f in L1_files_ref_slitless_prism:
            L1_files_all.append(get_L1_config_dict(
                f,
                file_trunc=os.path.join(
                    os.path.basename(os.path.dirname(f)),
                    os.path.basename(f)
                ),
                descrip='Reference star slitless prism',
                calib_product='Dispersion model',
    ...
    if 'L1_files_ref_slit_prism' in locals():
        for f in L1_files_ref_slit_prism:
            L1_files_all.append(get_L1_config_dict(
                ...
                descrip='Reference star slit + prism',
                calib_product='Registered prism image of reference star',
                header_keys=table_header_keys
            ))
    ...
    if 'L1_files_target_slitless_prism' in locals():
        for f in L1_files_target_slitless_prism:
            L1_files_all.append(get_L1_config_dict(
                ...
                descrip='Target star slitless prism',
                header_keys=table_header_keys
            ))
    ...
    if 'L1_files_target_slit_prism' in locals():
        for f in L1_files_target_slit_prism:
            L1_files_all.append(get_L1_config_dict(
                ...
                descrip='Target star slit + prism',
                calib_product='Registered prism image of target star',
                header_keys=table_header_keys
            ))
    ...
    if 'L1_files_brightstand_dither' in locals():
        for f in L1_files_brightstand_dither:
            L1_files_all.append(get_L1_config_dict(
                ...
                descrip='Dithered bright standard star',
                calib_product='ND filter, Abs Flux',
                header_keys=table_header_keys
            ))
    # Faint standard star 
    if 'L1_files_faintstand_dither' in locals():
        for f in L1_files_faintstand_dither:
            L1_files_all.append(get_L1_config_dict(
                ...
                descrip='Dithered faint standard star',
                calib_product='Abs Flux',
                header_keys=table_header_keys
            ))
            ...
            with fits.open(f, mode='readonly') as hdul:
                if '3D' in hdul[1].header['CFAMNAME']:
                    L1_files_all.append(get_L1_config_dict(
                        ...
                        descrip='Standard star line spread function',
                        calib_product='Line spread function',
                        header_keys=table_header_keys
                    ))
                else:
                    L1_files_all.append(get_L1_config_dict(
                        ...
                        descrip='Standard star line spread function',
                        calib_product='Slit transmission map',
                        header_keys=table_header_keys
                    ))
    ...
    if 'L1_files_ref_satspot' in locals():
        for f in L1_files_ref_satspot:
            L1_files_all.append(get_L1_config_dict(
                ...
                descrip='Reference star DM satellite spot',
                calib_product='Wavelength zeropoint',
                header_keys=table_header_keys
            ))
    ...
    if 'L1_files_target_satspot' in locals():
        for f in L1_files_target_satspot:
            L1_files_all.append(get_L1_config_dict(
                ...
                descrip='Target star DM satellite spot',
                calib_product='Wavelength zeropoint',
                header_keys=table_header_keys
            ))
    ...
    if 'L1_files_refstar_science_analog' in locals():
        for f in L1_files_refstar_science_analog:
            L1_files_all.append(get_L1_config_dict(
                ...
                descrip='Reference star occulted science - analog',
                header_keys=table_header_keys
            ))
    ...
    if 'L1_files_targetstar_science_analog' in locals():
        for f in L1_files_targetstar_science_analog:
            L1_files_all.append(get_L1_config_dict(
                ...
                descrip='Target star occulted science with companion - analog',
                header_keys=table_header_keys
            ))
    ...
    if 'L1_files_refstar_science_pc' in locals():
        for f in L1_files_refstar_science_pc:
            L1_files_all.append(get_L1_config_dict(
                ...
                descrip='Reference star occulted science - photon counting',
                header_keys=table_header_keys
            ))
    ...
    if 'L1_files_targetstar_science_pc' in locals():
        for f in L1_files_targetstar_science_pc:
            L1_files_all.append(get_L1_config_dict(
                ...
                descrip=(
                    'Target star occulted science with companion - '
                    'photon counting'
                ),
                header_keys=table_header_keys
            ))
