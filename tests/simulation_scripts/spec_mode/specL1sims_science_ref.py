...
def create_occ_refstar_sim(outdir, Vmag, sptype, slit_name, slit_pos_mas, 
                          gain=200, exptime=200, nframes=5, output_dim=161, overfac=5, loc_x=512, loc_y=512):
    """
    ...
    for ii in range(nframes):
        ...
        png_fname, header_txt_fname = write_png_and_header(sim_occ_refstar, outdir, loc_x, loc_y, output_dim)
        time.sleep(1) # Wait 1 sec to prevent collision between filenames

if __name__ == '__main__':
    outdir = os.path.relpath('./spec_occ_refstar')
    if not os.path.exists(outdir):
        os.mkdir(outdir)
    create_occ_refstar_sim(outdir, Vmag=2.0, sptype='A0V', slit_name='R1C2', slit_pos_mas=320.0, 
