...
def test_expected_flux_ratio_noise():
    ...
    frn_image_nostarloc = compute_flux_ratio_noise(psfsub_dataset_rdi[0], nd_cal, star_dataset, halfwidth=3)
    frn_image_starloc = compute_flux_ratio_noise(psfsub_dataset_rdi[0], nd_cal, star_dataset, unocculted_star_loc=np.array([[17], [15]]), halfwidth=3)

    assert isinstance(frn_image_nostarloc, data.Image)
    assert isinstance(frn_image_starloc, data.Image)
    assert 'FRN_CRV' in frn_image_nostarloc.hdu_list
    assert 'FRN_CRV' in frn_image_starloc.hdu_list
    assert np.array_equal(frn_image_nostarloc.hdu_list['FRN_CRV'].data, frn_dataset_nostarloc[0].hdu_list['FRN_CRV'].data)
