...
@pytest.mark.e2e
def test_noisemap_calibration_from_l1_old(tvacdata_path, e2eoutput_path):
    ...
    nonlin_path = os.path.join(processed_cal_path, "nonlin_table_240322.txt")
    stack_arr_f = []
    data_path = os.path.join(tvacdata_path, 'TV-20_EXCAM_noise_characterization', 'noisemap_test_data', 'test_l1_data')
    for f in os.listdir(data_path):
        file = os.path.join(data_path, f)
        if not file.endswith('.fits'):
            continue
        stack_arr_f.append(file)
    # ensure same file ordering
    l1_data_filelist = stack_arr_f
    l1_data_filename = corgidrp.data.Dataset(l1_data_filelist[:1])[0].filename
    ...
    exptime_arr = []
    gain_arr = []
    for entry in split_params:
        exptime_arr.append(entry[0])
        gain_arr.append(entry[1])
    ...
    calibrate_darks_lsq(stack_arr, gain_arr, exptime_arr, kgain_arr, int(det_params.params['fwc_pp']), int(det_params.params['fwc_pp']),
