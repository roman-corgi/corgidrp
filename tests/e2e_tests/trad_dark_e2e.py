...
def test_trad_dark(tvacdata_path, e2eoutput_path):
    ...
    mean of several darks at the same EM gain and exposure time).  So this function compares the DRP's output of build_trad_dark()
    to TVAC's dark current map, which was generated by a similar process (mean of several darks taken at the same EM gain and exposure time).
    ...
    walker.walk_corgidrp(trad_dark_data_filelist, "", build_trad_dark_outputdir, template="build_trad_dark.json")
    ...
    # Load
    trad_dark = fits.getdata(generated_trad_dark_file) #data.Image(generated_trad_dark_file)
    
    ##### Check against TVAC traditional dark result (which was effectively the dark current map)
    TVAC_trad_dark = fits.getdata(TVAC_dark_path)
    clean_trad_dark = (trad_dark.data - fpn_dat/1.340000033378601 - cic_dat)/100
    import matplotlib.pyplot as plt
    plt.imshow(trad_dark.data - TVAC_trad_dark)
    plt.show()
