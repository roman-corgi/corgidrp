...
def test_l2b_to_l3(e2edata_path, e2eoutput_path):
    ...
    e2e_data_path = os.path.join(e2eoutput_path, "l2b_to_l4_noncoron_output", "l2b_input_data")
    if not os.path.exists(e2e_data_path):
        os.makedirs(e2e_data_path)

    e2eoutput_path = os.path.join(e2eoutput_path, "l2b_to_l4_noncoron_output", "l2b_to_l3_output")
    ...
    image_path = glob.glob(os.path.join(e2eoutput_path, "*_l1_.fits"))[0]
    ...
    for i in range(num_frames): 
        ...
        # Generate unique timestamp for each batch
        # Use current time and increment for uniqueness
        ...
        new_image.filename = f"cgi_0200001999001000001_{time_str}_l2b.fits"
    ...
    #Read in an L3 file
    # Find the actual L3 output file generated by the DRP walker
    l3_filenames = glob.glob(os.path.join(e2eoutput_path, "*_l3_.fits"))
    
    # Sort by modification time to get the most recent file
    l3_filenames.sort(key=os.path.getmtime)
    
    # Take the first file (should be the only one)
    l3_filename = l3_filenames[0]
...
def test_l3_to_l4(e2eoutput_path):
    ...
    e2einput_path = os.path.join(e2eoutput_path, "l2b_to_l4_noncoron_output", "l2b_to_l3_output")

    e2eoutput_path_l4 = os.path.join(e2eoutput_path, "l2b_to_l4_noncoron_output", "l3_to_l4_output")
    ...
    image_path = glob.glob(os.path.join(e2eoutput_path_l4, "*_l1_.fits"))[0]
    ...
    # Generate proper filename for flux calibration
    fluxcal_time = datetime.now()
    fluxcal_time_str = data.format_ftimeutc(fluxcal_time.isoformat())
    fluxcal_filename = f"cgi_0200001999001000000_{fluxcal_time_str}_abf_cal.fits"
    
    fluxcal_fac.save(filedir=e2eoutput_path_l4, filename=fluxcal_filename)
    ...
    # Find the actual L4 output file generated by the DRP walker
    l4_filenames = glob.glob(os.path.join(e2eoutput_path_l4, "*_l4_.fits"))
    
    # Sort by modification time to get the most recent file
    l4_filenames.sort(key=os.path.getmtime)
    
    # Take the first file (should be the only one)
    l4_filename = l4_filenames[0]
    ...
    assert combined_image.ext_hdr['FLXCALFN'] == fluxcal_filename
...
if __name__ == "__main__":
    ...
    e2edata_dir = '/Users/jmilton/Downloads/E2E_Test_Data/'
