...
def test_l2b_to_l3(e2edata_path, e2eoutput_path):
    ...
    e2e_data_path = os.path.join(e2eoutput_path, "l2b_to_l4_output", "l2b_input_data")
    if not os.path.exists(e2e_data_path):
        os.makedirs(e2e_data_path)

    e2eoutput_path = os.path.join(e2eoutput_path, "l2b_to_l4_output", "l2b_to_l3_output")
    ...
    astrom_input_dir = e2e_data_path

    #Create the mock dataset
    mocks.create_astrom_data(field_path=field_path, filedir=astrom_input_dir)
    
    # Find the actual astrometric data file generated by mocks.create_astrom_data
    astrom_data_files = glob.glob(os.path.join(astrom_input_dir, "*_l1_.fits"))
    if not astrom_data_files:
        raise FileNotFoundError("No astrometric data files found")
    
    # Sort by modification time to get the most recent file
    astrom_data_files.sort(key=os.path.getmtime)
    image_path = astrom_data_files[0]
    ...
    # Generate proper filename for astrometric calibration
    current_time = datetime.now()
    time_str = current_time.strftime("%Y%m%dt%H%M%S")
    astrom_cal_filename = f"cgi_0200001999001000000_{time_str}_ast_cal.fits"
    
    astrom_cal.save(filedir=e2eoutput_path, filename=astrom_cal_filename)
    ...
    for ibatch in range(nbatch): 
        ...
        batch_time = base_time.replace(second=(base_time.second + ibatch) % 60, minute=(base_time.minute + ((base_time.second + ibatch) // 60)))
        time_str = batch_time.strftime("%Y%m%dt%H%M%S")
        
        new_image.filename = f"cgi_0200001999001000{ibatch:03d}_{time_str}_l2b.fits"
    ...
    # Generate unique timestamp for satellite spot image
    satspot_time = base_time.replace(second=(base_time.second + nbatch) % 60, minute=(base_time.minute + ((base_time.second + nbatch) // 60)))
    satspot_time_str = satspot_time.strftime("%Y%m%dt%H%M%S")
    sat_spot_image.filename = f"cgi_0200001999001000{nbatch:03d}_{satspot_time_str}_l2b.fits"
    ...
    l2b_data_filelist = sorted(glob.glob(os.path.join(e2e_data_path, "*_l2b.fits")))
    # Use the specific l2b_to_l3 recipe template instead of automatic generation
    template_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))),"corgidrp","recipe_templates","l2b_to_l3.json")
...
def test_l3_to_l4(e2eoutput_path):
    ...
    e2einput_path = os.path.join(e2eoutput_path, "l2b_to_l4_output", "l2b_to_l3_output")
    if not os.path.exists(e2einput_path):
        os.makedirs(e2einput_path)

    e2eoutput_path_l4 = os.path.join(e2eoutput_path, "l2b_to_l4_output", "l3_to_l4_output")
    ...
    # Find the actual astrometric data file generated by mocks.create_astrom_data
    astrom_data_files = glob.glob(os.path.join(e2eoutput_path_l4, "*_l1_.fits"))
    if not astrom_data_files:
        raise FileNotFoundError("No astrometric data files found")
    ...
    # Generate proper filename for astrometric calibration
    astrom_time = datetime.now()
    astrom_time_str = astrom_time.strftime("%Y%m%dt%H%M%S")
    astrom_filename = f"cgi_0200001999001000000_{astrom_time_str}_ast_cal.fits"
    
    astrom_cal.save(filedir=e2eoutput_path_l4, filename=astrom_filename)
    ...
    # Generate proper filename for core throughput calibration
    ct_time = datetime.now()
    ct_time_str = ct_time.strftime("%Y%m%dt%H%M%S")
    ct_filename = f"cgi_0200001999001000000_{ct_time_str}_ctp_cal.fits"
    
    ct_cal_tmp.save(filedir=e2eoutput_path_l4, filename=ct_filename)
    ...
    fluxcal_time_str = fluxcal_time.strftime("%Y%m%dt%H%M%S")
    ...
    # Find the actual L3 output file generated by the DRP walker
    l3_filenames = glob.glob(os.path.join(e2einput_path, "*_l3_.fits"))
    
    # Sort by modification time to get the most recent file
    l3_filenames.sort(key=os.path.getmtime)
    
    if not l3_filenames:
        raise FileNotFoundError("No L3 files found")
    
    l3_data_filelist = l3_filenames
    ...
    assert psf_subtracted_image.ext_hdr['CTCALFN'] == ct_filename
    assert psf_subtracted_image.ext_hdr['FLXCALFN'] == fluxcal_filename
