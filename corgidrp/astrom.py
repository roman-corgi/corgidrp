...
def boresight_calibration(input_dataset, field_path='JWST_CALFIELD2020.csv', field_matches=None, find_threshold=10, fwhm=7, mask_rad=1, 
    ...
    for i in range(len(dataset)):
        ...
        astrom_data = np.array([ra, dec, cal_properties[0], cal_properties[0], cal_properties[1], 0, 0, np.inf, np.inf])
    ...
    astromcal_data = np.concatenate((np.array([corr_pos_ra, corr_pos_dec, avg_platescale, avg_platescale, avg_northangle, avg_ra, avg_dec]), np.array(distortion_coeffs), np.array([order])), axis=0)
