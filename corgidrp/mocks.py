...
def create_nonlinear_dataset(nonlin_filepath, filedir=None, numfiles=2,em_gain=2000):
    ...
    for i in range(numfiles):
        ...
        #Apply the non-linearity to the data. When we correct we multiply, here when we simulate we divide
...
def generate_mock_pump_trap_data2(output_dir,meta_path, nonlin_path=None):
    """
    Generate mock pump trap data, save it to the output_directory
    ...
    # dc = {180: 0, 190: 0, 195: 0, 200: 0, 210: 0, 220: 0}
    bias = 1000 #0
    eperdn = 6 #1 Apply after simulating all traps, along with nonlin if applicable
    bias_dn = bias/eperdn
    nbits = 14 #1
    inj_charge = 500 # 0
    def _ENF(g, Nem):
