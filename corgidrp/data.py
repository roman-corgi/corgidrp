...
class Image():
    ...
    def __init__(self, data_or_filepath, pri_hdr=None, ext_hdr=None, err = None, dq = None, err_hdr = None, dq_hdr = None, input_hdulist = None):
        ...
        if not 'RA_BORE' in self.pri_hdr:
            self.pri_hdr.set('RA_BORE', 0, 'Boresight RA')
        if not 'DEC_BORE' in self.pri_hdr:
            self.pri_hdr.set('DEC_BORE', 0, 'Boresight Dec')
        if not 'RA_APER' in self.pri_hdr:
            self.pri_hdr.set('RA_APER', 0, 'CGI Aperture RA')
        if not 'DEC_APER' in self.pri_hdr:
            self.pri_hdr.set('DEC_APER', 0, 'CGI Aperture Dec')
